...
\begin{equation*}
T = \frac{HW}{rk}\left(L + \frac{k}{B} + wk\right) + \frac{(H - r)(r - 1)}{2}w
\end{equation*}

We can plug the expression of $k$ found in equation \ref{eq:up_r} and derive with respect to $r$:

\begin{equation}
\frac{dT}{dr} = \frac{-1}{r^2}\left(HL\left(p + \frac{1}{2}\right) + HW\left(\frac{1}{B} + w\right)\right) + \frac{w(H + 1)}{2} - wr
\end{equation}


As we do not know the parameters of the network, let us approximate some parts of the equation:

\begin{itemize}
    \item $p + \frac{1}{2} \simeq p$ ($p$ is large: highly distributed)
    \item $H + 1 \simeq H$ (The image is "big")
    \item $\frac{1}{B} + w \simeq w$ (The network is not the bottleneck of the algorithm)
\end{itemize}

At this point we have:

\begin{equation}
\frac{dT}{dr} = \frac{-1}{r^2}\left(HLp + HWw\right) + \frac{wH}{2} - wr
\end{equation}

We can also assume that $Lp + Ww \simeq Ww$ (The network is not the bottleneck of the algorithm).

We would like to solve $\frac{dT}{dr} = 0$ to find the value of $r$ minimizing $T$, and then compute the optimal value of $k$.

The equation to solve is thus:

\begin{equation}
r^3 - r^2\frac{H}{2} + HW = 0
\end{equation}
...
r <- roots[1] # The real root
r
k <- W / (r * (p - 0.5) + (p + 0.5))
k
```

We have to take the $r$ the closest to this optimal value such that $W \equiv 0~ (mod~ r\times p)$.
In this case $r = 32$.
...
So, in this example, we would take $r = 32$ and $k = 4$
...
```{r, echo=FALSE, fig.cap="\\label{fig:opti}Speedup depending on the block size ($r$)"}
