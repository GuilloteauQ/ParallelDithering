...
output:
    ...
    html_document:
        keep_tex: true
        number_sections: true
abstract: "From palette reduction in GIF images to printing, dithering techniques are widely used. With the incresing size of the images today, due to better photographic hardware, we can wonder if we can gain in performance by processing an image in parallel. In this paper, we will implement a MPI algorithm to apply the Floyd-Steinberg dithering in parallel. We will then focus on performance and look for optimal values of parameters."
---
...
The action of dithering is used in image processing to reduce the number of color used in an image.
One example is for printers that have only two values: either with ink or without.

The error diffusion in the dithering process consists to spread to neighbouring pixels the quantification error due to the restriction of a pixel to a reduced palette.
This technique reduces phenomenons such as banding (i.e. inaccurate color presentation) but introduces some noise.
...
Even with only two pixel values, the contrast of the different parts of of the image is repescted.
...
\begin{itemize}
    ...
    \item A script taking the design of the experiment and running it.
\end{itemize}

The first one will generate a CSV file composed of all the configurations to benchmark during the experiment.
The second will read the first one and then run the correct mpi commands to run the experiment.
...
On decision made to simplify the code was to encode pixels on a ``int16_t`` integer.

Of course, pixels in a grey scale image only go from 0 to 255.
Only requiring 8 bits.
...
\begin{enumerate}
    \item The first pixel of line $i$ is processed on process $P_{k}$. We add the proportions of the error to the corresponding cells of the buffer.
    ...
    \item The first cell of the buffer is ready to be send to process $P_{k+1}$ (no more depencies). It contains the cumulated error for the first pixel of line $i+1$.
...
The global time spent "busy waiting" is $2(H-1)w$ and we have to wait for the last process to finish.

We thus have:

\begin{equation}
T(n, p) = \frac{HW}{p} \left( w + L + \frac{1}{B}\right) + 2(H-1)w
\end{equation}

The sequential time is: $w\times H \times W$.

We can thus compute the speedup:

$$
\displaystyle S(n, p) = \frac{wHW}{\frac{HW}{p} \left( w + L + \frac{1}{B}\right) + 2(H-1)w}
$$

The limit speedup is thus:

\begin{equation}
\displaystyle \lim_{n \rightarrow \infty} S(n, p) = \frac{1}{\frac{1}{p} + \frac{L}{wp} + \frac{1}{wpB}}
\end{equation}
...
\begin{equation}
T(n, p) = \frac{n^2}{p \times k} \left( w \times k + L + \frac{k}{B}\right) + 2k(H - 1)w
\end{equation}

We compute next the speedup of this new version:

$$
\displaystyle S(n, p) = \frac{wHW}{\frac{n^2}{p\times k} \left( w \times k + L + \frac{k}{B}\right) + 2k(H-1)w}
$$

The limit speedup is thus:

\begin{equation}
\displaystyle \lim_{n \rightarrow \infty} S(n, p) = \frac{1}{\frac{1}{p} + \frac{L}{wpk} +\frac{1}{wpB}}
\end{equation}
...
\begin{equation}
\label{eq:eff2}
\displaystyle Eff(n, p)  = \lim_{n \rightarrow \infty} \frac{S(n, p)}{p} = \frac{1}{1 + \frac{L}{wk} + \frac{1}{Bw}}
\end{equation}
...
We cannot however afford to process the pixels in a block of lines line by line.
...
\begin{equation}
T(n, p) = \frac{HW}{krp} \left( wk + L + \frac{k}{B}\right) + wk\frac{(H-r)(r-1)}{2}
\end{equation}

We compute next the speedup of this new version:

$$
\displaystyle S(n, p) = \frac{wHW}{\frac{HW}{krp} \left( wk + L + \frac{k}{B}\right) + wk\frac{(H-r)(r-1)}{2}}
$$

The limit speedup is thus:

\begin{equation}
\displaystyle \lim_{n \rightarrow \infty} S(n, p) = \frac{1}{\frac{1}{pr} + \frac{L}{pkrw} + \frac{1}{pwrB}}
\end{equation}
...
\begin{equation}
\label{eq:eff2}
\displaystyle Eff(n, p)  = \lim_{n \rightarrow \infty} \frac{S(n, p)}{p} = \frac{1}{\frac{1}{r} + \frac{L}{wkr} + \frac{1}{Brw}}
\end{equation}
...
$$
\min \frac{(H - r)(r - 1)}{2}k
$$

We can plug the expression of $k$ found in equation \ref{eq:up_r} and derive with respect to $r$:

$$
\min \frac{(H - r)(r - 1)W}{2\left(r\left(p - \frac{1}{2}\right) + p + \frac{1}{2}\right)}
$$

\begin{equation}
\frac{dT_{w}}{dr} = \frac{(H+1-2r)(r(p-\frac{1}{2}) + p + \frac{1}{2}) - (H-r)(r-1)(p+\frac{1}{2})}{(r(p-\frac{1}{2})+p+\frac{1}{2})^2}
\end{equation}

We are looing at when the upper part equals 0.
...
with $P = \frac{p + \frac{1}{2}}{p - \frac{1}{2}}$
...
For large values of $H$ we have $r \simeq \sqrt{2H}$
...
We generated points for some possible values of $r$ and computed the optimal value of $k$ associated with this value of $r$ (see equation \ref{eq:up_r}).
...
This could be due to how we approximated the derivative of $T$.
...
* $\displaystyle k \simeq \frac{W}{\sqrt{2H}\left(p - \frac{1}{2}\right) + p + \frac{1}{2}}$

# Performances

TODO:

* Fix the image size and make the number of processes grow
...
In this paper, we decided to limit the implementation to a "simpler" use.
Indeed, our implementation requires $H \equiv 0 \pmod k$ and $H \equiv 0~ (mod~ r\times p)$.
Those constraints are pretty limiting.
This is why all the experiments are based on powers of 2.

By only using image with a size that is a power of 2, we limit ourselves to a very small region of the space.
...
* Working on blocks of pixels instead of single pixels $\rightarrow$ reduced the impact of the latency of the network

* Giving several consecutive lines to each process $\rightarrow$ reduced the impact of the network (latency + bandwith)

* Computing optimal values of $k$ and $r$ $\rightarrow$ those values are computed to reduce waiting time
